...
- **useWebGpuListen**: Speech recognition composable. Captures microphone audio via AudioWorklet, runs energy-based VAD, and transcribes finalized speech segments using Whisper large-v3 on WebGPU within `withGpuLock`. On transcription completion, text is sent to chat inference. During voice recognition, text input is disabled.
  - **No barge-in**: while a previous turn is still being transcribed, generated by the LLM, or spoken by TTS, microphone input is dropped and VAD state is reset, so a single utterance cannot spawn a second ASR/LLM run. The busy condition is supplied by `index.vue` via the `isBusy` option (`chat.isLoading || tts.isSpeaking`), combined with the composable's internal `isTranscribing`.
